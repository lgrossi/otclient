...
#define ROWBYTES(pixel_bits, width) \
    ((pixel_bits) >= 8 ? ((width) * (((unsigned int)(pixel_bits)) >> 3)) : ((((width) * ((unsigned int)(pixel_bits))) + 7) >> 3))
    ...
    for (j = 0; j < h; j++)
    {
        ...
        for (i = 0; i < w; i++)
        {
            ...
            case 8:
                ...
                break;
            case 4:
                col = (sp[i >> 1] & mask4[i & 1]) >> shift4[i & 1];
                break;
            case 2:
                col = (sp[i >> 2] & mask2[i & 3]) >> shift2[i & 3];
                break;
            case 1:
                col = (sp[i >> 3] & mask1[i & 7]) >> shift1[i & 7];
                break;
            }
